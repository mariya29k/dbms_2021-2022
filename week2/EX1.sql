...
SELECT LASTNAME, EDLEVEL, SALARY + 1,200 AS "NEW-SALARY", DECIMAL(BONUS *0.5,9,2) AS "NEW-BONUS"
FROM EMP
...
FROM EMP
WHERE WORKDEPT = 'D11' AND (SALARY + COMM + BONUS) > SALARY * 1.1
...
AND HIREDATE > BIRTHDATE + 16 YEARS
ORDER BY AGE, EMPNO;
...
SELECT PROJNO, DECIMAL(ROUND((DAYS(PRENDATE) - DAYS(PRSTDATE)) / 7.0,1), 8, 1) AS "WEEKS"
FROM PROJECT
